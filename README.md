...
| `forceRemeasureInNextLayout` | notify the layout manager that the dimensions of the views have changed. By default, the layout manager measures only one child and then assumes same dimensions for the rest of the views. Calling this method will remeasure that one standard view |
| `findFirstVisibleItemPosition`, `findFirstCompletelyVisibleItemPosition`, `findLastVisibleItemPosition`, and `findLastCompletelyVisibleItemPosition` | return the corresponding position of view as described in the fucntion's name |
